...
QPixmap CommonUtils::getRoundImage(const QPixmap& src,QPixmap& mask,QSize masksize) {
	...
	QImage reusltImage(masksize,QImage::Format_ARGB32_Premultiplied);
	QPainter painter(&reusltImage);
	painter.setCompositionMode(QPainter::CompositionMode_Source);
	painter.fillRect(reusltImage.rect(),Qt::transparent);
	...
	return QPixmap::fromImage(reusltImage);
}
